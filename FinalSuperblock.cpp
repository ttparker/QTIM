#include "FinalSuperblock.h"
    ...
    #define obsProjectNNCoupling projectNNCoupling
#endif
                                 ...
                                 std::vector<TheBlock>& rightBlocks)
{
    sysBlockOps.clear();
    envBlockOps.clear();
    lFreeSite = rFreeSite = Matrix<obsScalar, d, d>::Identity();
...
obsVectorX_t FinalSuperblock::actSysBlock(std::vector<TheBlock>& leftBlocks)
{
    ...
    obsMatrixX_t oPsi = rhoBasisRep(sysBlockOps, leftBlocks, lS) * psiGround;
                                                   ...
                                                   leftBlocks)
{
    ...
    obsMatrixX_t oSOLPsi = rhoBasisRep(sysBlockOps, leftBlocks, lS) * oLPsi;
    oSOLPsi.resize(mS * d * mE * d, 1);
    return oSOLPsi;
};

obsVectorX_t FinalSuperblock::actEnvBlock(std::vector<TheBlock>& rightBlocks)
{
    int mSd = mS * d;
    ...
    obsMatrixX_t oDagPsiT
        = rhoBasisRep(envBlockOps, rightBlocks, lE).adjoint() * psiT;
...
obsVectorX_t FinalSuperblock::actEnvBlockRFreeSite(std::vector<TheBlock>&
                                                   rightBlocks)
{
    obsMatrixX_t psiOEDag = actEnvBlock(rightBlocks);
                                          ...
                                          int blockSize) const
{
            ...
            rhoBasisBlockOp
                = kp(rhoBasisBlockOp, 
                     (   currentOp != opsEnd
                      ...
                      currentOp++ -> second :
                      Matrix<obsScalar, d, d>::Identity())).eval();
    }
    ...
    else
    {
        currentSite = currentOp -> first - 1;
        rhoBasisBlockOp
            = blocks[currentSite++].obsProjectFreeSiteOp(currentOp++ -> second);
    };
    for(int end = blockSize - 1; currentSite < end; currentSite++)
        ...
        rhoBasisBlockOp
            = (currentOp != opsEnd && currentSite == currentOp -> first - 1 ?
               blocks[currentSite].obsProjectNNCoupling(rhoBasisBlockOp,
                                                        currentOp++ -> second) :
               blocks[currentSite].obsProjectBlockOp(rhoBasisBlockOp));
